...
public class Server {
    ...
    public Server(String... orbArgs) throws InvalidName {
        orb = ORB.init(orbArgs, null);
        rootPOA = POAHelper.narrow(orb.resolve_initial_references(ROOT_POA_REFERENCE));
    ...
    public void deregister(String[] path) throws UserException {
        ...
        rootContext.resolve(pathList.toArray(new NameComponent[pathList.size()]));
        System.out.println("Sino - Serviço fora...");
    }

    public void start() throws AdapterInactive {
        rootPOA.the_POAManager().activate();
        System.out.println("Sino - Serviço no Ar...");
        orb.run();
    }
